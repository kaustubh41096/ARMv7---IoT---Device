...
void sendDhcpMessage(uint8_t packet[], uint8_t type)
{
    ...
    uint8_t reqip[IP_ADD_LENGTH] = {192,168,1,132 };
    ...
    else if (type == 3)
    {
        ...
        oplen = oplen
                + setDhcpOption(dhcp->options, 55, (uint8_t *) &parameters,
                                sizeof(parameters), oplen);
        oplen = oplen + setDhcpOption(dhcp->options, 54, (uint8_t *) &dhcpsm.siaddr, sizeof(dhcpsm.siaddr), oplen);
        oplen = oplen + setDhcpOption(dhcp->options, 12, (uint8_t *)&tmpHostIdentifier, sizeof(tmpHostIdentifier), oplen);
        oplen = oplen
                + setDhcpOption(dhcp->options, 255, &end, sizeof(end), oplen);
        udp->sourcePort = htons(68);
        udp->destPort = htons(67);
        dhcplen = sizeof(dhcpFrame) + oplen - 2;
        udplen = dhcplen + 8;
        udp->length = htons(udplen);dhcp->xid = 0x00000001;
        dhcp->xid = 0x00000001;
        dhcpsm.xid = 0x00000001;
        ip->flagsAndOffset = htons(0);
        ip->length = htons(20);
